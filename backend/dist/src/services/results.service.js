...
exports.computeStudentResult = computeStudentResult;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
async function computeStudentResult(studentId, termId) {
    ...
    for (const subject of subjects) {
        ...
        const assessments = [1, 2, 3, 4, 5, 6].map(n => {
            const m = subjectMarks.find(m => m.markType === 'ASSESSMENT' && m.assessmentNumber === n);
            return m?.score ?? null;
        });
        const assessmentTotal = assessments.reduce((sum, s) => sum + (s ?? 0), 0);
        // End-of-term score

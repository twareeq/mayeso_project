...
exports.generateClassReportsPDF = generateClassReportsPDF;
const pdfkit_1 = __importDefault(require("pdfkit"));
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
async function generateStudentReportPDF(studentId, termId, stream) {
...
async function generateClassReportsPDF(classId, termId, stream) {
    ...
    if (results.length === 0)
        throw new Error('No results found for this class and term');
    const doc = new pdfkit_1.default({ margin: 40 });
    doc.pipe(stream);
    for (let i = 0; i < results.length; i++) {
        ...
        for (let j = 0; j < headers.length; j++) {
            doc.text(headers[j], colX[j], doc.y, { continued: j < headers.length - 1 });
        }
        doc.moveDown(0.5);
        doc.lineWidth(0.5).moveTo(40, doc.y).lineTo(550, doc.y).stroke();
        doc.moveDown(0.5);
        ...
        doc.font('Helvetica-Bold');
        const rowY = doc.y;
        doc.text('TOTAL', colX[0], rowY);
        doc.text(totalA.toString(), colX[7], rowY);
        doc.text(totalExam.toString(), colX[8], rowY);
        doc.text(totalObtained.toString(), colX[9], rowY);
        doc.moveDown(2);
        // Summary
        doc.lineWidth(1).moveTo(40, doc.y).lineTo(550, doc.y).stroke();
        doc.moveDown(1);

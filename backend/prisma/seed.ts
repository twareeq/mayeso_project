import { PrismaClient, Role, LevelType, Gender, MarkType } from '@prisma/client';
...
async function main() {
  ...
  const lowLevel = await prisma.level.create({
    data: { name: 'Low Level', type: LevelType.LOW, schoolId: school.id },
  });
  const seniorLevel = await prisma.level.create({
    data: { name: 'Senior Level', type: LevelType.SENIOR, schoolId: school.id },
  });
  ...
  // 6. Users
  const passwordHashAdmin = await bcrypt.hash('Admin1234', 12);
  const passwordHashHead = await bcrypt.hash('Head1234', 12);
  const passwordHashTeacher = await bcrypt.hash('Teacher1234', 12);
  ...
  // 9. Students
  const studentNames = [
    { first: 'John', last: 'Banda', gender: Gender.MALE },
    { first: 'Mary', last: 'Phiri', gender: Gender.FEMALE },
    { first: 'James', last: 'Tembo', gender: Gender.MALE },
    { first: 'Grace', last: 'Mwale', gender: Gender.FEMALE },
    { first: 'Peter', last: 'Banda', gender: Gender.MALE },
  ];
